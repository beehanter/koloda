...
# 
# Создадим HTML-таблицу со ссылками на локальные файлы фотографий. При нажатии на ссылку фото должно открыться в новой вкладке.
...
def create_clickable_links(df: pd.DataFrame, photo_col: str) -> pd.DataFrame:
    ...
    # Убедимся, что путь к файлу в формате, понятном браузеру
    # file:// + абсолютный путь
    # Важно: Jupyter Lab должен быть запущен с доступом к корневой директории проекта
    ...
    def make_link(path):
        if not path:
            return ""
        # Составляем абсолютный путь к фото
        abs_photo_path = os.path.join(project_root, path)
        # Создаем URL
        file_url = Path(abs_photo_path).as_uri()
        return f'<a href="{file_url}" target="_blank">{os.path.basename(path)}</a>'
